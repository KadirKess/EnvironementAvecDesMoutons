...
import names # On va utiliser cette librairie pour donner des noms aléatoires aux moutons, aux loups, et aux chasseurs
...
# On génère un grand nombre de noms :
for i in range (100):
    nom = names.get_full_name("male")
    if nom[0] == 'M': # Ceux qui commencent par M sont pour les moutons
        noms_moutons.append(nom)
    elif nom[0] == 'L': #Ceux qui commencent par L sont pour les loups
        noms_loups.append(nom)
    elif nom[0] == 'C': #Ceux qui commencent par C sont pour les chasseurs
        noms_chasseurs.append(nom)
# On supprime les doublons :
noms_moutons = list(set(noms_moutons))
noms_loups = list(set(noms_loups))
noms_chasseurs = list(set(noms_chasseurs))
